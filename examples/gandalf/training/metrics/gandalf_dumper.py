...
class GandalfDumper(Dumper):
	...
	@staticmethod
	def get_threshold_by_order(df: pd.DataFrame, recall: int, score_col: str, label_col: str, quantile=None) -> float:
		if quantile:
	...
	def get_label_metric(self, df, threshold=None, score_col='output', label_col='label'):
		...
		for name, sub_df in df.groupby(by=self.tag_field):
			precision, recall, review_rate, leak_rate, err_rate, f1_measure, detail_set = \
				precision_rescall_score(sub_df.loc[:, score_col].tolist(),
	...
	def get_support(self, split_uv=True):
		...
		if not split_uv:
			...
			self.old_recall = df[(df[self.label_filed]==1)&(df[self.old_score_filed] >= self.old_low_threshold)].shape[0]
			return df
		else:
			high_df = df[(df.uv >= 100)]
			low_df = df[(df.uv < 100)]
			self.high_support = high_df[high_df[self.label_filed]==1].shape[0]# Consider the timeseries exempt, this label is not accurate
			self.low_support = low_df[low_df[self.label_filed]==1].shape[0]
			self.old_high_recall = high_df[(high_df[self.label_filed]==1)&(high_df[self.old_score_filed] >= self.old_high_threshold)].shape[0]
			self.old_low_recall = low_df[(low_df[self.label_filed]==1)&(low_df[self.old_score_filed] >= self.old_low_threshold)].shape[0]
	...
	def cal_threshold(self, split_uv=True):
		if not split_uv:
			...
			threshold, hit = self.get_threshold_by_order(df, recall=self.recall, score_col=self.output_field, label_col=self.label_filed)
			logging.info(
				f"Total:{df.shape[0]},\n"
			...
			high_threshold,high_hit = self.get_threshold_by_order(high_df, recall=self.old_high_recall,
														 score_col=self.output_field,
														 label_col=self.label_filed)
			low_threshold,low_hit = self.get_threshold_by_order(low_df, recall=self.old_low_recall,
														score_col=self.output_field,
														label_col=self.label_filed)
			logging.info(
	...
	def cal_label_metric(self, split_uv=True):
		if not split_uv:
			...
			old_tag_metric = self.get_label_metric(self.df, self.old_high_threshold,score_col=self.old_score_filed, label_col=self.label_filed)
			print(tabulate(old_tag_metric, headers='keys', tablefmt='simple_grid', colalign=('central',)))
			print("====Cal new metric====")
			df, threshold = self.cal_threshold(split_uv)
			tag_metric = self.get_label_metric(df, threshold,score_col=self.output_field, label_col=self.label_filed)
			print('====Show diff metric====')
			diff_metric = pd.merge(old_tag_metric,tag_metric,how='left',on=['name','support'])
			print(tabulate(diff_metric, headers='keys', tablefmt='simple_grid', colalign=('central',)))
		else:
			high_df, high_threshold, low_df, low_threshold = self.cal_threshold(split_uv)
			print("====Cal old high uv metric====")
			old_high_tag_metric = self.get_label_metric(high_df, self.old_high_threshold,score_col=self.old_score_filed, label_col=self.label_filed)
			print('====Cal new high uv metric====')
			high_tag_metric = self.get_label_metric(high_df, high_threshold,score_col=self.output_field, label_col=self.label_filed)
			print('====Show high uv diff metric====')
			high_diff_metric = pd.merge(old_high_tag_metric,high_tag_metric,how='left',on=['name','support'])
			print(tabulate(high_diff_metric, headers='keys', tablefmt='simple_grid', colalign=('central',)))
			...
			old_low_tag_metric = self.get_label_metric(low_df, self.old_low_threshold,score_col=self.old_score_filed, label_col=self.label_filed)
			print('====Cal new low uv metric====')
			low_tag_metric = self.get_label_metric(low_df, low_threshold,score_col=self.output_field, label_col=self.label_filed)
			print('====Show low uv diff metric====')
			low_diff_metric = pd.merge(old_low_tag_metric,low_tag_metric,how='left',on=['name','support'])
			print(tabulate(low_diff_metric, headers='keys', tablefmt='simple_grid', colalign=('central',)))
			# old_recall, new_recall = 0, 0
...
if __name__ == '__main__':
	dumper = GandalfDumper(["object_id","verify_reason","uv","online_score","output","label"], '/mlx_devbox/users/jiangxubin/repo/121/maxwell/id2score_2022102-7-9--2022103-0-1--2022110-1-2-.csv', strict=True)
