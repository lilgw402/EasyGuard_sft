...
class LinearLrSchedulerWithWarmUp(WarmUpLrScheduler):
    ...
    def get_lr(self):
        ...
        return [base_lr * (1 + (self.end_factor - self.start_factor)*(self.last_epoch-self.warmup_epoch)/(self.total_iters-self.warmup_epoch))
                for base_lr in self.base_lrs]

@SCHEDULERS.register_module()
class CosineLrSchedulerWithWarmUp(WarmUpLrScheduler):
    def __init__(self, optimizer, warmup_steps: int, last_epoch: int = -1, verbose=False,num_cycles: float = 0.5, min_factor: float = 0.0, total_iters=5,**kwargs):
    ...
    def get_lr(self):
        ...
        if self.last_epoch > self.total_iters:
            return [group['lr'] for group in self.optimizer.param_groups]
        progress = float(self._step_count - self.warmup_steps) \
                   / float(max(1, self.total_iters - self.warmup_steps))
        return max(0.0, self.min_factor + 0.5 * (1.0 - self.min_factor)* (1.0 + math.cos(math.pi * float(self.num_cycles) * 2.0 * progress)))

@SCHEDULERS.register_module()
class ExponentialLrSchedulerWithWarmUp(WarmUpLrScheduler):
    def __init__(self,optimizer,warmup_steps: int,last_epoch=-1,verbose=False,gamma=1.0,total_iters=5,**kwargs):
