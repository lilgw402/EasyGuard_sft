...
class EcomLiveGandalfAutoDisNNAsrSimcseCruiseModel(GandalfCruiseModule):
    ...
    def trace_step(self, batch):
        auto_dis_input,feature_dense,input_ids,attention_mask,token_type_ids = batch
        trace_output = self.forward(auto_dis_input,feature_dense,input_ids=input_ids,attention_mask=attention_mask,token_type_ids=token_type_ids)
        return trace_output
