...
def process_text(text, use_random=False):
        if not use_random:
            if 'deberta' in asr_encoder_config:
                ...
                asr_inputs = asr_tokenizer(text, max_length=asr_max_length, padding='max_length', truncation=True, return_tensors="pt")
            ...
            asr_inputs = {'input_ids': torch.tensor([0.5 for i in range(asr_max_length)], dtype=torch.int32).reshape(1, -1).cuda(), 
                          'attention_mask': torch.tensor([0.5 for i in range(asr_max_length)], dtype=torch.int32).reshape(1, -1).cuda(), 
...
def local_model_jit_inference(traced_model_file):
    ...
    prob = model(auto_dis_input, feature_dense, author_embedding,input_ids, attention_mask,token_type_ids)
    print(prob)
