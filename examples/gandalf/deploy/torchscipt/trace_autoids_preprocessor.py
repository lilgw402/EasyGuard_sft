...
class AutoDisPreprocessor(torch.nn.Module):
    ...
    def forward(
        self,
        features# 仅处理离散特征，embedding/直接输入，不需要预处理，cv/nlp在其他地方预处理
    ):
...
def trace_autodis_preprocessor(traced_preprocessor_model_file):
    module = AutoDisPreprocessor(feature_num=feature_num, active_slot=active_slot, min_max_mapping=min_max_mapping).cuda()
