...
class SOP(BaseDataset):
    def __init__(self, root, mode, train_split='Ebay_train.txt', transform = None):
        ...
        metadata = open(os.path.join(self.root, train_split if self.classes == range(0, 11318) else 'Ebay_test.txt'))
...
class MySOP(torch.utils.data.Dataset):
    def __init__(self, root, mode, train_split='Ebay_train.txt', transform = None):
        ...
        metadata = open(os.path.join(self.root, train_split if self.mode=='train' else 'Ebay_test.txt'))
