...
class NextWordDataProcessor:
    def __init__(self, tokenizer: str, max_seq_len:int, finetune_type_is_qa: bool, drop_last: bool = False, use_loss_mask: bool = False, stride=-1, **kwargs):
    ...
    def transform(self, data_dict):
        ...
        if self.use_loss_mask:
            if self.finetune_type_is_qa:
                ...
                text_dict['input_ids'] = [
                    [self.tokenizer.bos_token_id] + prompt_output_question['input_ids'] + prompt_output_answer['input_ids'] + [self.tokenizer.eos_token_id]
                ]
                ...
                if 'is_instruct' not in text_dict:
                    text_dict['loss_mask'] = [
                        [0] + [0] * len(prompt_output_question['attention_mask']) + [1] * len(prompt_output_answer['attention_mask']) + [1]
                    ]
                else:
                    if text_dict['is_instruct'] == False:
                        ...
                        text_dict['loss_mask'] = [
                            [1] + [1] * len(prompt_output_question['attention_mask']) + [1] * len(prompt_output_answer['attention_mask']) + [1]
                        ]
    ...
    def group_texts(self, examples):
        ...
        if mod_rst:
            if total_length < self.max_seq_len:
                for key in concatenated_examples:
                    ...
                    concatenated_examples[key] = [pad_token_id] * (self.max_seq_len-ori_total_length) + concatenated_examples[key]
                total_length = self.max_seq_len
            elif not self.drop_last:
                if self.stride!=-1:
                    n_subseqs = ceil((ori_total_length-self.max_seq_len) / self.stride) + 1
                    for key in concatenated_examples:
                        subseqs = [concatenated_examples[key][i*self.stride:i*self.stride+self.max_seq_len] for i in range(n_subseqs)]
        ...
        for i in range(0, total_length, self.max_seq_len):
            result = {
                k: torch.as_tensor(t[i : i + self.max_seq_len]) for k, t in concatenated_examples.items()
            }
...
class ZeroShotGPTDatamodule(CruiseDataModule):
    ...
    def get_train_steps(self):
        if self.hparams.dyn_bsz: #按照token bsz算
            eff_token_rate = 0.95
            train_size = int(self.hparams.train_size  * (1 + self.hparams.bsz_warmup_rate / 2) / eff_token_rate * (self.hparams.max_seq_len / self.hparams.stride))
            self.rank_zero_info(f"Estimated Train Size: {train_size}")
            train_steps = int(train_size // (self.hparams.train_batch_size * DIST_ENV.world_size * self.hparams.max_seq_len) )
        else:
