...
from .xperf_training import FTFlashAttention, FTLinear, FTLayerNorm, FTLinearWeightTransposed, Compress_input, Restore_output, Get_valid_word_index, Get_seq_len
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.use_rmpad_attn = config.get('use_rmpad_attn', False)
        self.use_rmpad_lnmlp = config.get('use_rmpad_lnmlp', False)
    ...
    def forward(
        ...
        bsz, *_, seq_len = attention_mask.shape
        if encoder_hidden_states is not None:
            ...
            if self.use_rmpad_attn and not self.use_rmpad_lnmlp: # if h is not compressed, compress it
                hidden_states = Compress_input(hidden_states, word_idx)
            query, key, value = self.c_attn(hidden_states).split(self.split_size, dim=-1)
            if self.use_rmpad_attn:
                query = Restore_output(query.contiguous(), word_idx, bsz, seq_len)
                key = Restore_output(key.contiguous(), word_idx, bsz, seq_len)
                value = Restore_output(value.contiguous(), word_idx, bsz, seq_len)
        ...
        if self.reorder_and_upcast_attn:
            ...
            if self.use_ft_flash_attn:
                ...
                attn_output = self.ft_flash_attn([query, key, value], self.num_heads, causal=True, attention_dropout=self.attn_pdrop, word_idx=word_idx, cu_seqlens_k=seq_lens, cu_seqlens_q=seq_lens, use_rmpad_attn=self.use_rmpad_attn)
                # if not self.use_rmpad_attn:
                # (batch, seq_length, head * head_features) -> (batch, seq_length, head, head_features)
        ...
        if self.use_rmpad_attn:
            attn_output = Compress_input(attn_output.contiguous(), word_idx)
        attn_output = self.c_proj(attn_output)
        attn_output = self.resid_dropout(attn_output)
        if self.use_rmpad_attn and not self.use_rmpad_lnmlp:
            attn_output = Restore_output(attn_output, word_idx, bsz, seq_len)
...
class GPT2Block(nn.Module):
    ...
    def forward(
        ...
        if self.use_rmpad_lnmlp and not self.use_rmpad_attn:
        ...
        # residual connection
        if self.use_rmpad_lnmlp and not self.use_rmpad_attn:
            attn_output = Compress_input(attn_output, word_idx)
        elif not self.use_rmpad_lnmlp and self.use_rmpad_attn:
            attn_output = Restore_output(attn_output, word_idx, bsz, seq_len)
        ...
        if self.gradient_checkpointing_mlp:
            feed_forward_hidden_states = torch.utils.checkpoint.checkpoint(
                self.mlp,
                hidden_states,
            )
        else:
...
class GPT2LMHeadModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.use_rmpad_lmloss = self.config.get('use_rmpad_lmloss', False)
    ...
    def forward(
        ...
        mask_for_idx = torch.logical_or(attention_mask, torch.flip(attention_mask, dims=[1])).float()
        word_idx = Get_valid_word_index(mask_for_idx)
        seq_lens = Get_seq_len(mask_for_idx)
        ...
        if labels is not None:
            ...
            if self.use_rmpad_lmloss:
                ...
                loss = self.loss_fct(lm_logits[:shift_labels.shape[0]], shift_labels)
            else:
