...
if __name__ == '__main__':
    ...
    item_lst = open('/mnt/bn/ecom-govern-maxiangqian/liuzeyu/hq_live/datasets/20221001_20221102/item_ids.txt').read().splitlines()
    num_workers = 16
    chunk_size = 32
    version = 'v1_0'
    prefix = 'train_86w'
    ...
    if split_trainval:
        run_processes_args_list(pack_data, train_item_lst, num_workers,
                                src_root_dir=src_root_dir, dst_root_dir=dst_root_dir, chunk_size=chunk_size)
        ...
        run_processes_args_list(pack_data, val_item_lst, num_workers,
                                src_root_dir=src_root_dir, dst_root_dir=dst_root_dir, chunk_size=chunk_size)
        print("Pack Val Dataset Done.")
        print(f"Merge into {dst_root_dir}...")
        merge_dataset(dst_root_dir, prefix + '_val', version)
    else:
        run_processes_args_list(pack_data, item_lst, num_workers,
                                src_root_dir=src_root_dir, dst_root_dir=dst_root_dir, chunk_size=chunk_size)
