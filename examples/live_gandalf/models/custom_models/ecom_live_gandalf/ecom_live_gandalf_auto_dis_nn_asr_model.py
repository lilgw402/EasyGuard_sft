...
#         print(type(batched_feature_data['feature_dense']),batched_feature_data['feature_dense'].shape)
#         print(type(batched_feature_data),type(batched_feature_data['asr']),batched_feature_data['asr'])
#         asr_inputs = self._asr_tokenizer(batched_feature_data["asr"])
#         print('asr_inputs',type(asr_inputs),asr_inputs[0].shape)
...
@MODELS.register_module()
class EcomLiveGandalfAutoDisNNAsrModel(LightningModule):
    def __init__(
        self,
        feature_num,
        bucket_num=8,
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        # Basic model config
        self._feature_num = self.hparams.feature_num
        self._bucket_num = self.hparams.bucket_num
        self._bucket_dim = self.hparams.bucket_dim
        self._bucket_output_size = self.hparams.bucket_output_size
        # Advanced model config
        self.model_kwargs = self.hparams.kwargs
        self._enable_asr_embedding = self.model_kwargs.get("enable_asr_embedding", 0)
        self._feature_input_num = self._feature_num - len(self.model_kwargs.get('slot_mask', []))
        self._drop_prob = self.model_kwargs.get('dropout', 0.3)
        self._bucket_all_emb_dim = self._feature_input_num * self._bucket_dim
        if self.model_kwargs.get("reset_params", False):
            self._reset_params()
        # Init encoders
        self.init_encoders()
        # Init loss weight
        self._loss_weight = self.model_kwargs.get('loss_weight', None)
        # Init metric
        self._metric = GeneralClsMetric()
        count_params(self)
    ...
    def init_encoders(self):
        self._auto_dis_bucket_encoder = AutodisBucketEncoder(
                                        ...
                                        add_block=False)
        # Init model components:asr
        self._asr_encoder_param = self.model_kwargs.get('asr_encoder_param', {})
