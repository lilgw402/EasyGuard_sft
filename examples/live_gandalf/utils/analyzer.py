...
def smape(preds, labels):
    return (
        2.0 * np.mean(np.abs(preds - labels) / (np.abs(preds) + np.abs(labels))) * 100
    )
...
def precision_rescall_score(preds, labels, threshold=0.5):
    ...
    inter_review_positive_count = np.sum(
        np.array(
            [
                1 if preds[i] > threshold and labels[i] == 1 else 0
    ...
    inter_review_negative_count = np.sum(
        np.array(
            [
                1 if preds[i] > threshold and labels[i] == 0 else 0
                for i in range(total_count)
            ]
        )
    )
    inter_review_count = (
        np.sum(np.array([1 if pred > threshold else 0 for pred in preds])) + 1e-20
    )
    total_positive_count = np.sum(labels) + 1e-20

    review_rate = (
        1.0 * inter_review_count / (total_count + 1e-5)
    )  # 进审率（影响面），预估大于阈值的样本量/样本总量
    review_detail = "({:.0f}/{:.0f})".format(inter_review_count, total_count)
    precision = (
        1.0 * inter_review_positive_count / (inter_review_count + 1e-5)
    )  # 审出率，进审的正样本量/进审样本量
    precision_detail = "({:.0f}/{:.0f})".format(inter_review_positive_count, inter_review_count)
    recall = (
        1.0 * inter_review_positive_count / (total_positive_count + 1e-5)
    )  # 召回率，进审的正样本量/正样本量
    ...
    err_detail = "({:.0f}/{:.0f})".format(inter_review_negative_count, inter_review_negative_count + inter_review_positive_count)
    detail_set = [precision_detail, recall_detail, review_detail, leak_detail, err_detail, f1_detail]
    return precision, recall, review_rate, leak_rate, err_rate, f1_measure, detail_set
...
def log_loss(preds, labels):
    ...
    for i in range(len(preds)):
        try:
            ...
            total_error += labels[i] * math.log(preds[i]) + (
                1.0 - labels[i]
            ) * math.log(1.0 - preds[i])
...
def pr_score_analysis(preds, labels, start=0, end=1, partition=250, fixed_recall=0.3, dump_table_name=None, hdfs_table_name=None):
    ...
    for i in range(partition):
        ...
        precision, recall, review_rate, leak_rate, err_rate, f1, details = precision_rescall_score(
            preds, labels, threshold=threshold
        )
        precision_detail, recall_detail, review_detail, leak_detail, err_detail, f1_detail = details
...
def fix_recall_analysis(preds, labels, fixed_recall=0.3):
    ...
    return {'threshold':f"{threshold:.4f}", 'review_rate':f"{review_rate:.2%}",'precision':f"{precision:.2%}",'recall':f"{recall:.2%}",'leak_rate':f"{leak_rate:.2%}",'err_rate':f"{err_rate:.2%}",'accuracy':f"{accuracy:.2%}",'f1':f"{f1:.4f}"}
