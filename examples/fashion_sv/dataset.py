...
class SVDataModule(CruiseDataModule):
    ...
    def setup(self, stage) -> None:
        self.train_dataset = SVDataset(
            self.hparams,
            data_path=self.hparams.train_files,
            is_training=True)

        self.val_dataset = SVDataset(
            self.hparams,
    ...
    def train_dataloader(self):
        sampler_train = DistributedSampler(
            self.train_dataset, num_replicas=int(os.environ.get('WORLD_SIZE') or 1),
            rank=int(os.environ.get('RANK') or 0), shuffle=True
        )
    ...
    def val_dataloader(self):
        sampler_val = DistributedSampler(
            self.val_dataset, num_replicas=int(os.environ.get('WORLD_SIZE') or 1),
            rank=int(os.environ.get('RANK') or 0), shuffle=False
        )
