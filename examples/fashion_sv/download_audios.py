...
def process_sample(line):
    ...
    for url in sample['audio_urls']:
        try:
            ...
            assert not os.path.exists(f"/mnt/bn/multimodel-pretrain/fashoin_sv/audio4sv/{sample['user_id']}/{filename}")
            with open(
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor(max_workers=10) as p:
        futures = [
            p.submit(download_by_file_mutlithread, file) for file in files
