...
def process_sample(line):
    ...
    if not os.path.exists(f"/mnt/bd/wx-nas/dataset/audio4sv/{sample['user_id']}"):
        try:
            os.makedirs(f"/mnt/bd/wx-nas/dataset/audio4sv/{sample['user_id']}")
        except:
            pass
    ...
    for url in sample['audio_urls']:
        try:
            ...
            assert not os.path.exists(f"/mnt/bd/wx-nas/dataset/audio4sv/{sample['user_id']}/{filename}")
            with open(f"/mnt/bd/wx-nas/dataset/audio4sv/{sample['user_id']}/{sample['room_id']}-{filename}", 'wb') as f:
                f.write(wav_data)
...
if __name__ == '__main__':
    ...
    files = glob('/mnt/bd/wx-nas/audiosamples/*.jsonl')
    print(len(files))
    start = time.time()
    # for file in files[:5]:
    #     download_by_file_mutlithread(file)
    with ProcessPoolExecutor(max_workers=10) as p:
        futures = [
            p.submit(download_by_file_mutlithread, file) for file in files[:50]
            if not os.path.exists(f"/mnt/bd/wx-nas/dataset/audio4sv/{file.split('/')[-1]}")
        ]
        wait(futures)
