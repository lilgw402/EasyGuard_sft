...
class GPT2Model(CruiseModule):
    ...
    def on_train_start(self) -> None:
        ...
        if global_config['trainer']['resume_ckpt_path'] and global_config['data']['dyn_bsz']:
            ...
            self.trainer._current_step = max(0, self.trainer._current_step - global_config['data']['train_num_workers']*global_config['data']['max_seq_len']*coeff)
    ...
    def count_tokens(self, attention_mask):
        ...
        if self.trainer._datamodule.hparams.dyn_bsz:
            bsz = min((self.trainer.global_step+1) / bsz_warmup_rate * train_steps, train_batch_size)
        else:
            bsz = train_batch_size
        seq_len = self.trainer._datamodule.hparams.max_seq_len

        self.average_token_rate = (self.average_token_rate*self.trainer.global_step + batch_tokens/bsz/seq_len) / (self.trainer.global_step+1)
        ...
        if self.trainer.global_step % self.trainer._log_every_n_steps == 0 and self.trainer.global_rank==0:            
            ...
            self.trainer.logger.log_metrics({'consume_tokens(B)': self.consume_tokens / 1e9}, step=self.trainer.global_step, step_size=None, reduce_fx=lambda x: x[0] if isinstance(x, list) else x, pause_flush=True)
            self.trainer.logger.log_metrics({'average_token_rate': self.average_token_rate * 100}, step=self.trainer.global_step, step_size=None, reduce_fx=lambda x: x[0] if isinstance(x, list) else x, pause_flush=True)
            self.trainer.logger.log_metrics({'tokens_per_second(M)': tokens_per_second / 1e6}, step=self.trainer.global_step, step_size=None, reduce_fx=lambda x: x[0] if isinstance(x, list) else x, pause_flush=True)
    ...
    def on_train_batch_start(self, batch, batch_idx):
        if batch_idx < self.resume_training_step:
            ...
            if batch_idx % 200 == 0:
                self.rank_zero_info(f"Resuming Data State through iterating data: {batch_idx} / {self.resume_training_step}")
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, Conv1D)):
            ...
            module.weight.data.normal_(mean=0.0, std=math.sqrt(2/(self.hparams.network.vocab_size + self.hparams.network.n_embed)))
        ...
        for name, p in module.named_parameters():
            ...
            if 'pre_token_proj.weight' in name:
                p.data.normal_(mean=0.0, std=math.sqrt(2/(self.hparams.network.n_embed + self.hparams.network.hidden_size)))
    ...
    if cfg.val_only:
        ...
        if cfg.play_file:
            ...
            if cfg.play_file_type == "qa":
                play_file_qa(cfg.play_file, tokenizer, model.cuda(), cfg.generate_trial_num,
            ...
            play_console(tokenizer, model.cuda(), cfg.generate_trial_num,
                         steps=cfg.generate_steps, temperature=cfg.generate_temp, do_sample=cfg.generate_do_sample,
                         top_k=cfg.generate_topk, top_p=cfg.generate_topp,dynamic_top_p = cfg.generate_dynamic_topp,
                         omega = cfg.generate_dynamic_topp_omega,decay_lambda=cfg.generate_dynamic_topp_decay, 
                         until_n_eos=cfg.generate_n_eos)
    else:
