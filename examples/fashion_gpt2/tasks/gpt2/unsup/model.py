...
class GPT2Model(CruiseModule):
    ...
    def on_train_start(self) -> None:
        ...
        if global_config['trainer']['resume_ckpt_path'] and global_config['data']['dyn_bsz']:
            ...
            self.trainer._current_step = max(0,
                                             self.trainer._current_step - global_config['data']['train_num_workers'] *
                                             global_config['data']['max_seq_len'] * coeff)
    ...
    def count_tokens(self, attention_mask):
        ...
        if self.trainer.global_step % self.trainer._log_every_n_steps == 0 and self.trainer.global_rank == 0:
            ...
            self.trainer.logger.log_metrics({'consume_tokens(B)': self.consume_tokens / 1e9},
                                            step=self.trainer.global_step, step_size=None,
                                            reduce_fx=lambda x: x[0] if isinstance(x, list) else x, pause_flush=True)
            self.trainer.logger.log_metrics({'average_token_rate': self.average_token_rate * 100},
                                            step=self.trainer.global_step, step_size=None,
                                            reduce_fx=lambda x: x[0] if isinstance(x, list) else x, pause_flush=True)
            self.trainer.logger.log_metrics({'tokens_per_second(M)': tokens_per_second / 1e6},
                                            step=self.trainer.global_step, step_size=None,
                                            reduce_fx=lambda x: x[0] if isinstance(x, list) else x, pause_flush=True)
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, Conv1D)):
            ...
            module.weight.data.normal_(mean=0.0, std=math.sqrt(
                2 / (self.hparams.network.vocab_size + self.hparams.network.n_embed)))
...
if __name__ == '__main__':
    ...
    cli.add_argument('--generate-dynamic-topp', default=None, type=float,
