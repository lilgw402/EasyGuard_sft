...
from mariana.models.vit import VisualTransformer
...
class CLIPDyCoverViTAlbertTower(CruiseModule):
    """ViT + Albert 双塔"""
    def __init__(self,
                 bert: CruiseConfig = albert_default_config,
                 large_proj: bool = False,
                 # Visual
                 visual_model: str = 'vit',
                 # ViT
                 vit: CruiseConfig = vit_default_config,
                 # NCE
                 tau: float = 0.07,
        ...
        vit = self.hparams.vit
        if self.hparams.visual_model.lower() == 'vit':
            self.visual = VisualTransformer(vit.patch_length, vit.patch_size,
                                            vit.embed_dim, vit.num_layers, vit.num_heads,
                                            visual_output_dim, vit.mlp_dim,
                                            dropout=vit.dropout,
                                            emb_dropout=vit.emb_dropout,
                                            layernorm_eps=vit.layernorm_eps,
                                            use_checkpoint=vit.use_checkpoint,
                                            use_native_attention=vit.use_native_attention)
        else:
            raise ValueError(f"unknown visual model: {visual_model}")
        total_visual_params = sum(
            [v.numel() for v in self.visual.parameters()])
        print(f'{self.hparams.visual_model} Params: {total_visual_params}')
        ...
        self.calc_nce_loss = LearnableNTXentLoss(
            init_tau=self.hparams.tau, clamp=self.hparams.nce_clamp)
    ...
    def encode(
        ...
        if mode == 't':
            ...
            if len(image.shape) == 4:
                encoder_output = self.visual(image)
