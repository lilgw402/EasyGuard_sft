...
@torch.no_grad()
def beam_search(model_decode_func, x, steps, beam_size=2, length_penalty_alpha=0.7,
                eos=3, until_n_eos=1):
    """
    Take a conditioning sequence of indices in x (of shape (b,t)) and predict the next token in
    the sequence, feeding the predictions back into the model each time.
...
def sample_generate(model, input_ids,
    ...
    # 3. 生成结果
    decoded_sentence = sample(model_decode_func=model_decode_func,
                              x=input_ids,
                              steps=steps,
...
def play_file_qa(fname, tokenizer, model, output_file_path,  trial_num=5,
    ...
    with open(fname, mode='r', encoding='utf-8') as fr, \
        ...
        for i, line in enumerate(tqdm(fr, desc=f"PROCESSING FILE: {output_file_path}")):
            try:
                jl = json.loads(line)
                ...
                for i in range(trial_num):
                    ...
                    completion = ''.join(tokenizer.decode(y))
                    completion.replace('##', '')
                    fw.write("\t".join([text, label_name, label, completion]) + '\n')
            except Exception as e:
                print(e)
                print(traceback.format_exc())
            ...
            for line in f:
                ...
                try:
                    ...
                    for i in range(trial_num):
                        ...
                        res = model.decode(input_ids=input_ids.to(model.device), input_mask=input_mask)
