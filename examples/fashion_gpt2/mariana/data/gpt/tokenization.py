...
def get_emoji_regex_pattern(mode="py"):
  ...
  if mode == "tensorflow":
    single_codes = [r"\x{{{}}}".format(
      hex(emoji_tuple[0])[2:]) for emoji_tuple in _full_emoji_codes if
      len(emoji_tuple) == 1]

    codes_range = [
      r"[\x{{{}}}-\x{{{}}}]".format(
        hex(emoji_tuple[0])[2:], hex(emoji_tuple[1])[2:])
      for emoji_tuple in _full_emoji_codes if len(emoji_tuple) == 2]
    return "|".join(single_codes + codes_range)

  single_codes = [chr(emoji_tuple[0]) for emoji_tuple in _full_emoji_codes
                  if len(emoji_tuple) == 1]
  codes_range = ["[{}-{}]".format(chr(emoji_tuple[0]), chr(emoji_tuple[1]))
                 for emoji_tuple in _full_emoji_codes if len(emoji_tuple) == 2]
  return "|".join(single_codes + codes_range)
...
class EmojiPadding(BaseCaster):
  ...
  def __init__(self):
    self.regex_pattern = re.compile(
      "({})".format(get_emoji_regex_pattern("purepy")))
...
class PunctuationPadding(BaseCaster):
  ...
  def _is_punctuation(char):
    ...
    # consistency.
    if ((33 <= char_ord <= 47) or (58 <= char_ord <= 64) or (
        91 <= char_ord <= 96) or (123 <= char_ord <= 126)):
      return True
...
class CaseNormalizer(BaseCaster):
  ...
  def __init__(self, do_lowercase=False, unicode_norm=None):
    if unicode_norm and unicode_norm.upper() not in {"NFD", "NFC", "NFKD",
                                                     "NFKC"}:
...
class Libcut(BaseCaster):
  ...
  def __init__(self, libcut_config, cut_mode="search"):
    ...
    try:
        ...
        logging.error("Unable to import `Cutter` from `text_cutter.cut`,"
    ...
    if not self.cut_mode.startswith("libcut_v2"):
      raise ValueError("Caster with purepy backend cannot run "
                       "when cut_mode is not libcut_v2*")
    ...
    self.cutter = Cutter(
      self.cut_type,
      resource_path=self.libcut_config)
...
class SentencePieceBPE(BaseCaster):
  ...
  def _tokenize(self, token_list):
    ...
    for token in token_list:
      if self.max_tokens_per_input > 0 and len(
          output_tokens) > self.max_tokens_per_input:
        break
...
class WordpieceTokenizer(BaseCaster):
  ...
  def __init__(self, vocab, unk_token="[UNK]", wordpiece_type="bert",
               max_input_chars_per_word=200, use_bpe=True,
               max_tokens_per_input=-1, **kwargs):
  ...
  def _lookup(self, token_list):
    def token2idx(token):
      ...
      if idx == self.unk_id and self.num_oov_buckets > 0:
        import farmhash
        idx = farmhash.fingerprint64(token) % self.num_oov_buckets + \
          len(self.vocab)
      return idx
          ...
          for key in libcut_config:
            if isinstance(libcut_config[key], str):
              ...
              if key != "SEARCH_DATA_PATH":
                new_val = os.path.join(libcut_data_path,
                                       "/".join(val.split("/")[-2:]))
              else:
                new_val = os.path.join(libcut_data_path, val.split("/")[-1])
              logging.info(
                "Change libcut path from {} to {}.".format(val, new_val))
...
class CasterTokenizer:
    ...
    def __init__(self, caster_configs):
        ...
        for caster_name, caster_params in caster_configs:
            if "vocab_str" in caster_params:
                ...
                self.vocab_dict = {token: idx for idx, token in
                                    enumerate(vocab_str.split("\n"))}
    ...
    def convert_tokens_to_ids(self, tokens):
        ...
        assert any(isinstance(caster, VocabLookup) for caster in self.casters), "Unable to convert tokens to ids due to missing VocabLookup"
        unsqueeze = False
    ...
    def convert_ids_to_tokens(self, ids):
        ...
        assert any(isinstance(caster, VocabLookup) for caster in self.casters), "Unable to convert ids to tokens due to missing VocabLookup"
        for caster in self.casters:
            if isinstance(caster, VocabLookup):
