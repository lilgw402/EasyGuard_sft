...
class DistLineReadingDataset(IterableDataset):  # pylint: disable=W0223
    ...
    def __init__(self,
        ...
        self.files = [f for f in self.files if f.find('_SUCCESS') < 0 and not os.path.basename(f).startswith('_')]
        self.files.sort()
        ...
        if len(self.files) % self.world_size != 0:
            logging.info('[DATA]--Whole dataset file num %s cannot split to worldsize %s ' %
                     (len(self.files), self.world_size))
        self.verbose = verbose
        if data_size < 0:
            logging.warn("Unknown data size, assume 1000000")
        self.data_size = data_size if data_size > 0 else 10000000

    def __len__(self):
        return self.data_size // self.world_size
        ...
        while True:
            ...
            if worker_info is not None:
                if len(cur_dataloader_files) % worker_info.num_workers != 0 and self.verbose:
                    logging.info('[DATA]--current dataloader [%s] file num %s cannot split to worker_num %s ' %
