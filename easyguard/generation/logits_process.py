...
class NoBadWordsLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, bad_words_ids: List[List[int]], eos_token_id: int):
        ...
        if any(
            not isinstance(bad_word_ids, list) for bad_word_ids in bad_words_ids
        ):
            raise ValueError(
        ...
        bad_words_ids = list(
            filter(
                lambda bad_token_seq: bad_token_seq != [eos_token_id],
                bad_words_ids,
            )
        )
    ...
    def __call__(
        ...
        if (
            self.static_bad_words_mask is None
            and len(self.bad_words_id_length_1) > 0
        ):
            self.static_bad_words_mask = self._calc_static_bad_word_mask(scores)

        dynamic_banned_tokens = self._calc_banned_bad_words_ids(
            input_ids.tolist()
        )
    ...
    def _calc_static_bad_word_mask(
        ...
        for prev_input_ids_slice in prev_input_ids:
            ...
            for banned_token_seq in self.bad_words_id_length_greater_than_1:
                if self._tokens_match(
                    prev_input_ids_slice, banned_token_seq[:-1]
                ):
            ...
            if banned_mask_list:
                ...
                banned_mask = (
                    torch.sparse.LongTensor(
                        banned_mask.t(), indices, scores.size()
                    )
                    .to(scores.device)
                    .to_dense()
                    .bool()
...
class HammingDiversityLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        if not isinstance(diversity_penalty, float) or (
            not diversity_penalty > 0.0
        ):
            raise ValueError(
    ...
    def __call__(
        ...
        group_end_idx = min(
            group_start_idx + self._num_sub_beams, self._num_beams
        )
