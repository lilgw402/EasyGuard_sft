...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def step_sentence_constraint(
        ...
        full_hypotheses = torch.cat(
            (input_ids[sent_beam_indices], sent_beam_tokens.unsqueeze(-1)),
            dim=-1,
        )
        ...
        for seq_idx, pre_seq in enumerate(this_batch_input_ids):
            ...
            if not advance_state.completed:
                advance_tokens = torch.LongTensor(advance_state.advance()).to(
                    device
                )
        ...
        if len(track_new["new_indices"]) > 0:
            ...
            all_banks = torch.tensor([one.get_bank() for one in all_states]).to(
                device
            )
            ...
            sent_beam_indices = torch.cat((sent_beam_indices, new_indices))[
                indices
            ]
