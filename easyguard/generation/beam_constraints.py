...
class DisjunctiveConstraint(Constraint):
    ...
    def __init__(self, nested_token_ids: List[List[int]]):
        ...
        if any(
            not isinstance(token_ids, list) for token_ids in nested_token_ids
        ):
            raise ValueError(
...
class ConstraintListState:
    ...
    def add(self, token_id: int):
        if not isinstance(token_id, int):
            raise ValueError(
                f"`token_id` should be an `int`, but is `{token_id}`."
            )
        ...
        if self.inprogress_constraint is not None:
            ...
            stepped, complete, reset = self.inprogress_constraint.update(
                token_id
            )
            if reset:
            ...
            for cidx, pending_constraint in enumerate(self.pending_constraints):
                if pending_constraint.does_advance(token_id):
                    stepped, complete, reset = pending_constraint.update(
                        token_id
                    )
    ...
    def copy(self, stateful=True):
        ...
        if stateful:
            ...
            if self.inprogress_constraint is not None:
                new_state.inprogress_constraint = (
                    self.inprogress_constraint.copy(stateful=True)
                )
