...
from cruise.data_module import create_cruise_loader, customized_processor
...
class HighQualityLiveProcessor:
    def __init__(self, test_mode,
        ...
        self.text_process = TextProcessor(vocab_file=vocab_file, max_len={'text_ocr':ocr_max_len,'text_asr':asr_max_len})
    ...
    def transform(self, data_dict: dict):
        # parse text input
        token_ids = self.text_process({"text_ocr":data_dict['ocr_text'],"text_asr":data_dict['asr_text']})
        # parse frame input
    ...
    def batch_transform(self, data):
        ...
        for ib, ibatch in enumerate(data):
            ...
            input_ids.append(ibatch['token_ids'][:max_len] +
                             [self.PAD] * (max_len - len(ibatch['token_ids'])))
            input_mask.append([1] * len(ibatch['token_ids'][:max_len]) +
        ...
        batch = {"frames": torch.stack(frames, dim=0), 
...
class HighQualityLiveDataModule(CruiseDataModule):
    ...
    def train_dataloader(self):
        return create_cruise_loader(
            ...
            processors=HighQualityLiveProcessor(False, ocr_max_len=self.hparams.ocr_max_len, asr_max_len=self.hparams.asr_max_len, frame_len=self.hparams.frame_len),
    ...
    def val_dataloader(self):
        return create_cruise_loader(
            ...
            shuffle=False,
            # predefined_steps=2000,
            processors=HighQualityLiveProcessor(True, ocr_max_len=self.hparams.ocr_max_len, asr_max_len=self.hparams.asr_max_len, frame_len=self.hparams.frame_len),
            decode_fn_list=[HighQualityLiveDataDecode()]
        )
