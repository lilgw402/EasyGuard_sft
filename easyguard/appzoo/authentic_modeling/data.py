...
class VideoFrameProcess(object):
    def __init__(self, test_mode=False, clip_len=1, frame_interval=1,
                 num_clips=4, keep_tail_frames=False, out_of_bound_opt='loop'):
...
class AuthenticProcessor:
    def __init__(self, test_mode,
                 ...
                 #           'video_ocr_is_title'),
                 frame_key='frame_file_paths', label_keys=('label', )):
...
class AuthenticDataModule(CruiseDataModule):
    ...
    def train_dataloader(self):
        return create_cruise_loader(
            ...
            processors=AuthenticProcessor(False, text_max_len=self.hparams.max_len, frame_len=self.hparams.frame_len, vocab_file=self.hparams.vocab_file),
    ...
    def val_dataloader(self):
        return create_cruise_loader(
            ...
            shuffle=False,
            # predefined_steps=2000,
            processors=AuthenticProcessor(True, text_max_len=self.hparams.max_len, frame_len=self.hparams.frame_len, vocab_file=self.hparams.vocab_file),
            decode_fn_list=[AuthenticDataDecode()]
        )
