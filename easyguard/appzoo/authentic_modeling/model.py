...
from ...utils.losses import LearnableNTXentLoss, LearnablePCLLoss, SCELoss, cross_entropy, ArcMarginProduct
...
class AuthenticMM(CruiseModule):
    ...
    def setup(self, stage) -> None:
        ...
        if self.config_visual.name == 'swin':
            ...
            self.visual = ConvNeXt(
                depths=self.config_visual.depths, 
                dims=self.config_visual.dims
            )
        
        if self.config_visual.pooling_type:
            self.frame_fusion = TemporalFusion(self.config_visual.pooling_type, self.config_visual.output_dim, self.config_visual.num_frames)
        ...
        self.fuse_category_lvl1 = nn.Sequential(
            ...
            nn.Linear(self.config_fusion.hidden_size * 2, self.config_fusion.category_logits_level1)
        )
        if self.hparams.use_multilabel:
            self.fuse_category_lvl2 = nn.Sequential(
                ...
                nn.GELU(),
                nn.Dropout(self.config_fusion.embd_pdrop),
                nn.Linear(self.config_fusion.hidden_size * 2, self.hparams.num_lvl2_labels)
            )
        ...
        if self.hparams.use_arcface:
            self.arc = ArcMarginProduct(in_features=fuse_emb_size, out_features=self.config_fusion.category_logits_level1)
        ...
        if self.config_fusion.name == 'transformer':
            ...
            if t_emb is not None and v_emb is not None:
                fuse_emb = self.fuse_dropout(
                    torch.cat([cls_emb, self.ln_visual(v_emb), self.ln_text(t_emb)], dim=1))  # [B, 1 + S_v + S_t, d_f]
            elif t_emb is not None:
                # 文本单模态
                fuse_emb = self.fuse_dropout(
                    torch.cat([cls_emb, self.ln_text(t_emb)], dim=1))  # [B, 1 + S_t, d_f]
            else:
                # 视觉单模态
                fuse_emb = self.fuse_dropout(
                    torch.cat([cls_emb, self.ln_visual(v_emb)], dim=1))  # [B, 1 + S_v, d_f]
    ...
    def cal_bce_loss(self, **kwargs):
        for key in ['out_lvl2', 'leaf_label']:
            if key in kwargs:
                ...
                kwargs[key] = kwargs[key].flatten(0, 1)
        # HARD CODE!!!
        pos_weight = torch.Tensor([1., 0.3, 0.3, 0.3, 1., 1., 1., 0.1, 0.1, 1., 1., 1., ]).to(kwargs['out_lvl2'].device)
        ###############
        loss = F.binary_cross_entropy_with_logits(kwargs['out_lvl2'], kwargs['leaf_label'].float(), pos_weight=pos_weight)
        return {
            "bce_loss": loss
        }
    ...
    def training_step(self, batch, idx):
        token_ids, segment_ids, attn_mask, image, label = batch['token_ids'], batch['segment_ids'], batch['attn_mask'], batch['frames'], batch['label']
        ...
        if self.hparams.use_multilabel:
            ...
            loss_dict = self.merge_loss([loss_dict, bce_loss_dict], [1., 1.])
        else:
            loss_dict = self.merge_loss([loss_dict,], [1.,])
    ...
    def configure_optimizers(self):
        ...
        normal_params_dict = {'params': [], 'weight_decay': self.hparams.weight_decay}
