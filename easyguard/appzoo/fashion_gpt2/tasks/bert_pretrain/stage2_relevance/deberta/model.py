...
from mariana.models.ptx_deberta import DebertaModel, init_weights
from mariana.optim import mariana_optimizer_kwargs_defaults


# Config adapter
deberta_default_config = {
    "vocab_size": 145608,
    "embedding_dim": 256,
    "dim": 768,
    "dim_ff": 3072,
    ...
    "extra_da_transformer_config": {
      ...
      "use_ft_mm_in_attn_wo_scale": False,
...
class DebertaPretrainModel(CruiseModule):
    """Deberta pretrain"""
    def __init__(self,
                 bert: CruiseConfig = deberta_default_config,
                 freeze_prefix: Optional[List[str]] = None,
                 partial_pretrain: Optional[str] = 'hdfs://haruna/home/byte_arnold_lq/data/reckon/mlxlab_aml_test/tasks/3252601/trials/10617714/output/checkpoints/epoch=1-step=612792.ckpt',
        ...
        # 文本
        self.deberta = DebertaModel(**self.hparams.bert)
    ...
    def forward(
        ...
        return self.deberta.forward(
            input_ids=input_ids, position_ids=position_ids,
            segment_ids=segment_ids, attention_mask=input_mask,
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.deberta.bare_mode = False
        loss_dict = self.forward(**self._preprocess_batch(batch))
        if self.deberta.local_metrics:
            self.log_dict(self.deberta.local_metrics, console=True)
        # pn loss
        pooled_output = loss_dict.pop('pooled_output')
    ...
    def validation_step(self, batch, batch_idx):
        self.deberta._omit_other_output = True
        self.deberta.bare_mode = True
        ...
        if benchmark not in SUPPORTED_BENCHMARKS:
            ...
            batch['model'] = self
            batch['hidden_size'] = self.deberta.encoder.dim
