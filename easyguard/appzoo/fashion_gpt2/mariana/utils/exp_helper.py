...
class ExpHelper:
    ...
    def _parse_model_names(self, file_path):
        ...
        if hdfs_prefix:
            ...
            hdfs_prefix = os.path.join(
                hdfs_prefix, self.model_line, self.model_type,
                self.model_name, str(self.original_trial))
    ...
    def _get_trial_info(self):
        ...
        if arnold_trial_id != self.DEBUG_PREFIX:
            try:
                ...
                response = requests.get(f'http://arnold-api.byted.org/api/v2/trial/{arnold_trial_id}/', headers=headers)
    ...
    def report_trial_info(self, rank=None, world_size=None):
        ...
        if self.preemptible:
            logging.info(f"[Rank {rank}/{world_size}]In preemptible trial: {self.arnold_trial_id}, original trial: {self.original_trial}")
        elif self.arnold_trial_id != self.DEBUG_PREFIX:
            if self.details.get('debug', ''):
                logging.info(f"[Rank {rank}/{world_size}]In arnold trial: {self.original_trial}, debug mode: {self.details.get('debug')}")
            else:
