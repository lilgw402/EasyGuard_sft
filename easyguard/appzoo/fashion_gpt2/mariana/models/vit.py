...
class AttnConfig:
    ...
    def __init__(self, hidden_size: int, hidden_dropout_prob: float, num_attention_heads: int):
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, d_mlp: int, attn_mask: torch.Tensor = None,
                 dropout=0., layernorm_eps=1.0e-5, use_native_attention=False):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, d_mlp: int,
                 attn_mask: torch.Tensor = None, dropout=0., layernorm_eps=1.0e-5,
                 use_checkpoint=False, use_native_attention=True):
        super().__init__()
        self.width = width
        self.layers = layers
...
class VisualTransformer(nn.Module):
    ...
    def __init__(self, patch_length: int, patch_size: int, width: int, layers: int,
                 heads: int, output_dim: int, d_mlp: int = None, dropout=0.,
                 emb_dropout=0., layernorm_eps=1.0e-5, use_checkpoint=False,
        ...
        self.transformer = Transformer(width, layers, heads, d_mlp, dropout=dropout,
                                       layernorm_eps=layernorm_eps, use_checkpoint=use_checkpoint,
                                       use_native_attention=use_native_attention)
