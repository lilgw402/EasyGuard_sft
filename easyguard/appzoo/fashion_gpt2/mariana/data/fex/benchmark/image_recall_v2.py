"""Image recall benchmark v2, copied from fuxi benchmark, skip config loading"""
import os
import json
import logging
...
class ImageSearchRecallBenchmarkV2:
    def __init__(self, model, vocab_file, output_path,
                 data_path="hdfs://haruna/user/suiguobin/image/tusou/click_202107_val",
        ...
        self.seq_len = 24  # fixed
        self.query_batch_size = query_batch_size
        self.image_batch_size = image_batch_size
        self.data_size = data_size
        self.save_file_key = 'tuso_v2'
    ...
    def calc_recall_rate(self, topks=(1, 5, 10)):
        ...
        logging.info('Tusou Recall V2: calculating recall ...')
        ...
        logging.info('Tusou Recall V2: loaded query: %s, %s' % (len(queries), t_emb.shape))
        ...
        logging.info('Tusou Recall V2: loaded video: %s, %s' % (len(gids), v_emb.shape))
        ...
        result = {}
        s = '\n[Tuso Recall v2 (zero shot)]\n'

        for topk in topks:
            _, rank_vt = inter_mat.topk(topk, dim=0)  # [img_size, topk]
            rank_vt = rank_vt.t()
            _, rank_tv = inter_mat.topk(topk, dim=1)  # [cap_size, topk]
            # logging.info('Tusou recall V2: vgt {} tgt {}'.format(vgt[:topk], tgt[:topk]))
            # logging.info(f'Tusou Recall V2: matrix shape {inter_mat.shape} {rank_vt.shape} {rank_tv.shape}')
            vcnt = 0.
            ...
            result[f'tusou_v2_V2T_{topk}'] = round(vcnt / total * 100, 4)
            result[f'tusou_v2_T2V_{topk}'] = round(tcnt / total * 100, 4)
    ...
    def encode_query(self, is_save=True):
        ...
        rank0print(f'Tusou Recall V2: encoding query with bsz {batch_size} ...')
        ...
        logging.info(f'[Rank {DIST_ENV.rank}/{DIST_ENV.world_size}] Tusou Recall V2: query dataload len: {loader_len}')
        while True:
            ...
            if DIST_ENV.rank == 0 and i > 0 and i % 50 == 0:
                logging.info(f'Tusou Recall V2: query embs batch step {i}/{loader_len}')
            i += 1

        t_embs = torch.cat(t_embs, dim=0)
        logging.info('[Rank %d/%d] Tusou Recall V2: encoded query %s queries, %s embs' % (
    ...
    def encode_image(self, is_save=True):
        ...
        rank0print(f'Tusou Recall V2: encoding video with bsz {batch_size}...')
        if DIST_ENV.rank >= 128:
            logging.info("[Rank %d/%d] Tusou Recall V2: Skip due to oversize gpu worker.")
            return
        ...
        i = 0
        pre_data = pre_docs = None
        logging.info(f'[Rank {DIST_ENV.rank}/{DIST_ENV.world_size}] Tusou Recall V2: image dataloader len: {loader_len}')
        while True:
            data, has_data, shoud_stop = get_data_all_rank(data_iter, self.model)
            if shoud_stop:
                break
            ...
            if has_data:
                ...
                infos.extend(docs)
            if DIST_ENV.rank == 0 and i > 0 and i % 50 == 0:
                logging.info(f'Tusou Recall V2: image embs batch step {i}/{loader_len}')
            i += 1

        v_embs = torch.cat(v_embs, dim=0)
        logging.info(f'[Rank {DIST_ENV.rank}/{DIST_ENV.world_size}] Tusou Recall V2: encoded {len(infos)} images, {v_embs.shape} embs')
        if is_save:
