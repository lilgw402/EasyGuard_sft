...
class DebertaDataModule(CruiseDataModule):
    def __init__(
        ...
        self.local_pretrained_model_dir = (
            f"{self.hparams.local_dir_prefix}/{suffix_model}"
        )

        suffix_cutter = self.hparams.cutter_resource_dir.split("/")[-1]
        self.local_cutter_dir = (
            f"{self.hparams.local_dir_prefix}/{suffix_cutter}"
        )
    ...
    def local_rank_zero_prepare(self) -> None:
        ...
        if not os.path.exists(self.local_cutter_dir):
            os.system(
                f"hdfs dfs -copyToLocal {self.hparams.cutter_resource_dir} {self.hparams.local_dir_prefix}"
            )
    ...
    def setup(self, stage) -> None:
        ...
        self.vocab = Vocabulary(
            self.local_pretrained_model_dir + "/vocab.txt", oov_token="[UNK]"
        )
    ...
    def predict_dataloader(self):
        return DistributedCruiseDataLoader(
            ...
            predefined_steps=self.hparams.val_data_size
            // self.hparams.val_batch_size,
            source_types=["jsonl"],
            shuffle=False,
        )
