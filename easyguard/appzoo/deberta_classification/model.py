...
class DebertaModel(CruiseModule):
    def __init__(self,
        ...
        self.local_pretrained_model_dir = f"{self.hparams.local_dir_prefix}/{suffix}"
        ...
        self.num_labels = 2
        self.classifier = torch.nn.Linear(in_features=self.deberta.encoder.dim,
    ...
    def forward(self, input_ids, attention_mask, segment_ids, labels=None):
        ...
        pooled_output = self.deberta(
            input_ids, segment_ids=segment_ids, attention_mask=attention_mask, output_pooled=True
        )['pooled_output']
    ...
    def training_step(self, batch, idx):
        output_dict = {}
        input_ids, attention_mask, segment_ids, labels = batch['input_ids'], batch['attention_mask'], batch['segment_ids'], batch['labels']
    ...
    def configure_optimizers(self):
        ...
        # )
        optimizer = Optimizer.from_option(list(self.named_parameters()), {"type": "adam", "lr": self.hparams.learning_rate})
    ...
    def freeze_parameters(self, num_layers_frozen=6):
        ...
        for name, param in self.named_parameters():
            if any([name.startswith(pop_name) for pop_name in params_exclusive]):
    ...
    def predict_step(self, batch, idx):
        ...
        for label, predict, score in zip(labels.cpu().numpy().tolist(), pred_labels.cpu().numpy().tolist(), scores.cpu().numpy().tolist()):
