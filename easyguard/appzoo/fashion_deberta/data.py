...
class TextProcessor:
    ...
    def batch_transform(self, batch_data):
        ...
        for ib, ibatch in enumerate(batch_data):
            ...
            special_tokens_mask.append([1] + [0] * (len(ibatch['input_ids'][:max_len])-2) + [1] + [1] * (max_len - len(ibatch['input_ids'])))
        ...
        mlm_input_ids, mlm_labels = self.torch_mask_tokens(input_ids, special_tokens_mask)
...
class FashionDataModule(CruiseDataModule):
    ...
    def setup(self, stage) -> None:
        ...
        self.tokenizer = BpeTokenizer(self.local_pretrained_model_dir + "/vocab.txt",
                         wordpiece_type = "bert",
                         lower_case = False)
        self.vocab = build_vocab(self.local_pretrained_model_dir + "/vocab.txt")
        self.cutter = Cutter("CRF_LARGE", self.local_cutter_dir) if self.hparams.cutter_enable else None
