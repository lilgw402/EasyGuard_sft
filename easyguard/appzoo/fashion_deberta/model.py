...
class FashionDebertaModel(CruiseModule):
    ...
    def setup(self, stage) -> None:
        ...
        self.bert_config_path = os.path.join(
            self.local_pretrained_model_dir, "config.json"
        )
        self.config = DebertaConfig.from_json_file(self.bert_config_path)

        self.mlm_prediction = DebertaLMPredictionHead(self.config)
        self.cls_prediction = (
            Prediction(self.config.hidden_size * 2, self.hparams.cls_class_num)
            if self.hparams.cat_max_pool_enable
            else nn.Linear(self.config.hidden_size, self.hparams.cls_class_num)
        )
    ...
    def training_step(self, batch, idx):
        ...
        mlm_loss = self.loss_fct(
            mlm_prediction_logits.reshape(-1, self.config.vocab_size),
        ...
        if self.hparams.classification_task_enable:
            ...
            if self.hparams.cat_max_pool_enable:
                max_pooling_output = torch.max(
                    (output["sequence_output"] * input_masks.unsqueeze(-1)),
                    axis=1,
                ).values  # [bz, hidden]
                classification_pred_logits = self.cls_prediction(
                    output["pooled_output"], max_pooling_output
                )  # [bz, cls_class_num]
            else:
                classification_pred_logits = self.cls_prediction(
            ...
            self.train_return_loss_dict["loss"] = (
                cls_loss * self.hparams.cls_weight + mlm_loss
            )
        ...
        # TODO: support using cl and cls the same time
            ...
            self.train_return_loss_dict["loss"] = (
                cl_loss * self.hparams.cl_weight + mlm_loss
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.hparams.classification_task_enable:
            ...
            # acc
            acc_result = accuracy(
                mlm_prediction_logits,
                mlm_labels,
                classification_pred_logits,
            ...
            self.val_return_loss_dict["val_mlm_loss"] = mlm_loss
            self.val_return_loss_dict["val_loss"] = (
                cl_loss * self.hparams.cl_weight + mlm_loss
            )
...
class Linear(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.model = nn.Sequential(
            linear, nn.GELU(), LayerNorm(out_features, eps=self.layer_norm_eps)
        )
...
def accuracy(
    mlm_logits, mlm_labels, risk_logits=None, risk_label=None, PAD_IDX=-100
):
    """
