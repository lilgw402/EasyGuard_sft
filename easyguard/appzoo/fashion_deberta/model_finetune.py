...
class FashionDebertaFtModel(CruiseModule):
    ...
    def setup(self, stage) -> None:
        ...
        self.cls_prediction = nn.Linear(self.hparams.hidden_size, self.hparams.cls_class_num)
        
        self.cls_loss_fct = SCELoss(alpha=1.0, beta=0.5, num_classes=self.hparams.cls_class_num) \
                                if self.hparams.sce_loss_enable else nn.CrossEntropyLoss(ignore_index=-100)
    ...
    def training_step(self, batch, idx):
        try:
            input_ids, input_masks, input_segment_ids, classification_labels = batch['input_ids'], batch['input_masks'], batch['input_segment_ids'], batch['classification_labels']
