...
class FashionDebertaModel(CruiseModule):
    ...
    def setup(self, stage) -> None:
        ...
        self.cls_prediction = Prediction(self.hparams.hidden_size, self.hparams.cls_class_num) \
                                if self.hparams.hf_enable else nn.Linear(self.hparams.hidden_size, self.hparams.cls_class_num, self.hparams.hidden_size)
    ...
    def training_step(self, batch, idx):
        input_ids, input_masks, input_segment_ids, mlm_labels = batch['mlm_input_ids'], batch['input_masks'], batch['input_segment_ids'], batch['mlm_labels']
        if self.hparams.hf_enable:
            output = self.deberta(input_ids, input_masks, input_segment_ids, labels=mlm_labels, output_hidden_states=True)
