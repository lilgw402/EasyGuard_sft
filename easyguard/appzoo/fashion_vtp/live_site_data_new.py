...
class LiveSiteDataModule(CruiseDataModule):
    ...
    def test_dataloader(self):
        sampler_test = torch.utils.data.DistributedSampler(
            self.test_dataset, num_replicas=int(os.environ.get('WORLD_SIZE') or 1), rank=int(os.environ.get('RANK') or 0), shuffle=False
        )
        return torch.utils.data.DataLoader(
                                self.test_dataset,
...
class MMDataset(Dataset):
    ...
    def __init__(self, params, data_path, is_training=False):
        ...
        for sample in fin.readlines():
            ...
            label1, label2 = data_item[self.label_key[0]], data_item[self.label_key[1]]
    ...
    def __getitem__(self, index):
        ...
        input_dict = {'vid': self.vids[index],
                      'frames': frames,
                      'input_ids': token_ids,
    ...
    def text_preprocess(self, texts):
        ...
        for text_type in self.text_types:
            ...
            # tokens += self.tokenizer.tokenize(text[:self.max_len[text_type]])[:self.max_len[text_type] - 2] + ['[SEP]']
