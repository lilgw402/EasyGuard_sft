...
if USE_JAX in ENV_VARS_TRUE_AND_AUTO_VALUES:
    ...
    if _flax_available:
        try:
            ...
            _flax_version = importlib.metadata.version("flax")
...
try:
    ...
    _ = importlib.metadata.version("datasets")
    _datasets_metadata = importlib.metadata.metadata("datasets")
...
try:
    _detectron2_version = importlib.metadata.version("detectron2")
...
try:
    _ftfy_version = importlib.metadata.version("ftfy")
...
try:
    _coloredlogs_available = importlib.metadata.version("coloredlogs")
...
try:
    _sympy_available = importlib.metadata.version("sympy")
...
try:
    _tf2onnx_version = importlib.metadata.version("tf2onnx")
...
try:
    _onxx_version = importlib.metadata.version("onnx")
...
try:
    _pytorch_quantization_version = importlib.metadata.version("pytorch_quantization")
...
try:
    _soundfile_version = importlib.metadata.version("soundfile")
...
try:
    _tensorflow_probability_version = importlib.metadata.version("tensorflow_probability")
...
try:
    _timm_version = importlib.metadata.version("timm")
...
try:
    _natten_version = importlib.metadata.version("natten")
...
try:
    _torchaudio_version = importlib.metadata.version("torchaudio")
...
try:
    _phonemizer_version = importlib.metadata.version("phonemizer")
...
try:
    _pyctcdecode_version = importlib.metadata.version("pyctcdecode")
...
try:
    _librosa_version = importlib.metadata.version("librosa")
...
try:
    _decord_version = importlib.metadata.version("decord")
...
class _LazyPackage(ModuleType):
    """
    Package class that surfaces all objects but only performs associated imports when the objects are requested.
    """
