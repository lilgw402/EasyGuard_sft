...
class RouteFuncMLP(nn.Module):
    ...
    def __init__(self, c_in, c_out, mid_dim, kernels, bn_eps=1e-5, bn_mmt=0.1, concat=True):
        """
        Args:
        ...
        self.b.weight.data.zero_()  # to make sure the initial values  for the output is 1.

    def forward(self, cc_emb, cls_emb, v_mask):
        calibrate_emb = (
            torch.cat([cls_emb, self.g(cc_emb).expand(-1, -1, cls_emb.shape[2], -1, -1)], dim=1)
            if self.concat
            else cls_emb + self.g(cc_emb)
        )
        calibrate_emb = torch.einsum("bcshw,bs->bcshw", calibrate_emb, v_mask.type(calibrate_emb.dtype))
...
class TAdaConv2d(nn.Module):
    ...
    def __repr__(self):
        return (
            f"TAdaConv2d({self.in_channels}, {self.out_channels}, kernel_size={self.kernel_size}, "
...
class VideoAdapter(nn.Module):
    def __init__(
        self, embed_dim=512, mid_dim=64, conv_mid=16, n_head=2, attn_mask=None, scale=0.1, pos=None, placeholder=True
    ) -> None:
        super().__init__()
        ...
        self.cc = nn.Parameter(mid_dim**-0.5 * torch.randn(mid_dim))
    ...
    def forward(self, x: Tensor, bs, mask=None):
        ...
        patch_num = int((x.shape[0] - 1) ** 0.5)
        cls_down = cls_down.reshape(bs, -1, self.mid_dim)
        patch_down = patch_down.reshape(patch_num, patch_num, bs, -1, self.conv_mid).permute(
            2, 4, 3, 0, 1
        )  # bs, e, frames, pn, pn
        ...
        cls_down = torch.cat([self.cc.to(dt) + torch.zeros(bs, 1, mid_dim, dtype=dt, device=device), cls_down], dim=1)
        ...
        final_mask = self.block.attn(cls_down, cls_down, cls_down, need_weights=False, attn_mask=attn_mask_)[0]

        cls_down = self.block((cls_down, None, None, 0, None), final_mask)[0]
        cls_down = cls_down.permute(1, 0, 2)  # bs, frames + 1, e_a
        alpha = self.conv_rf(
            cls_down[:, 0][..., None, None, None], cls_down[:, 1:].permute(0, 2, 1)[..., None, None], v_mask
        )
        # patch_down = self.conv_tada(patch_down.float(), alpha).to(dt).permute(
        patch_down = (
            self.conv_tada(patch_down, alpha).to(dt).permute(3, 4, 0, 2, 1).reshape(patch_num**2, -1, self.conv_mid)
        )

        cls_down = self.act(cls_down)
        patch_down = self.act(patch_down)
        delta_x = torch.cat([self.up(cls_down[:, 1:].reshape(1, -1, mid_dim)), self.patch_up(patch_down)])
...
class ResidualAttentionAdapterBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask=None, seq_len=32, adapt=None, pos=None, idx=0):
        super().__init__()
    ...
    def attention(self, x: torch.Tensor):
        txt_len = x.shape[0]
        attn_mask = (
            self.attn_mask.to(dtype=x.dtype, device=x.device)[:txt_len, :txt_len]
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        self.ln_2 = LayerNorm(d_model)
        self.attn_mask = attn_mask
        self.video_adapter, self.img_adapter, self.text_adapter = nn.Identity(), nn.Identity(), nn.Identity()
...
class ConvAdapter(nn.Module):
    def __init__(
        self, embed_dim=512, mid_dim=16, mid_k=5, xavier_init=False, scale=0.1, dropout=0.0, placeholder=True
    ) -> None:
        super().__init__()
        self.placeholder = placeholder
        if placeholder:
        ...
        if xavier_init:
            ...
            self.conv.weight.data[:, :, 1, 1] += torch.eye(mid_dim, dtype=torch.float)
    ...
    def forward(self, x: torch.Tensor, video_frame=-1, bs=-1, mask=None):
        ...
        x = torch.cat(
            [
                ...
                # self.prompt.to(x.dtype) +
                # torch.zeros(x.shape[0], self.prompt_len, x.shape[-1], dtype=x.dtype, device=x.device),
                x,
            ],
            dim=1,
...
def build_adapter(
    ...
    if image_resolution == 336:  # use 224 to save memory
        ...
        src_cls = src[0:1]
        src_oth = src[1:]
        src_oth = F.interpolate(src_oth.reshape(24, 24, 1024).permute(2, 0, 1).unsqueeze(0), (16, 16), mode="bilinear")
    ...
    print(f"CLIP Missing_keys  {missing_keys}")
