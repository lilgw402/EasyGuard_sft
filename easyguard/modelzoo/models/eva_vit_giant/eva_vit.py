...
from .uni_video_adapter import VideoAdapter
...
class Block(nn.Module):
    ...
    def forward_ori(self, x, rel_pos_bias=None):
        x1 = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias))
        x2 = self.drop_path(self.gamma_2 * self.mlp(self.norm2(x1)))
        return x1, x2

    def forward(self, x, rel_pos_bias=None, batch_size=None):
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, bs=None):
        ...
        for blk in self.blocks:
            ...
            # else:
            x = blk(x, rel_pos_bias, batch_size=bs)
    ...
    def forward(self, x, bs=None):
        ...
        pooler = self.head(x[:, 0])
        rep = {"feature": x, "pooler": pooler}
        return rep
