...
class AttentionExpertLinearTranspose(nn.Module):
    ...
    def _shape(self, tensor: torch.Tensor, bsz: int) -> torch.Tensor:
        ...
        return (
            tensor.view(bsz, -1, self.n_heads, self.head_dim)
            .permute(0, 2, 1, 3)
            .contiguous()
        )
...
class DisentangledMHA(nn.Module):
    ...
    def __init__(self, config: DAConfig, **kwargs):
        ...
        if self.config.use_moe_attn and str(
            ...
            self.proj_moe = janus.layer.MoE(
                ...
                expert=AttentionExpertLinearTranspose(
                    config.dim, config.dim, config.n_heads, config.head_dim
                ),
        ...
        self.pos_att_type = tuple(
            [x.strip() for x in config.pos_att_type.lower().split("|")]
            if config.pos_att_type
            else []
        )
    ...
    def _shape(self, tensor: torch.Tensor, bsz: int) -> torch.Tensor:
        """(bsz, seq_len, dim) -> (bsz, n_heads, seq_len, head_dim)"""
        return (
            tensor.view(bsz, -1, self.config.n_heads, self.config.head_dim)
            .permute(0, 2, 1, 3)
            .contiguous()
        )
    ...
    def disentangled_att_bias(
        ...
        assert (
            relative_pos.dim() == 4
        ), f"Relative position ids must be of dim 4 instead of {relative_pos.dim()}"
        ...
        if "c2p" in self.pos_att_type:
            ...
            c2p_att = torch.matmul(
                query_layer, pos_key_layer.permute(1, 2, 0)
        ...
        if "p2c" in self.pos_att_type:
            ...
            p2c_att = torch.matmul(
                key_layer, pos_query_layer.permute(1, 2, 0)
            )  # (bsz, n_heads, k_len, att_span*2)
    ...
    def _experimental_disentangled_att_bias(
        ...
        if "c2p" in self.pos_att_type:
            ...
            pos_key_layer = pos_key_layer.permute(
                1, 2, 0
            )  # (n_heads, head_dim, att_span*2)
            ...
            pos_key_layer = torch.gather(
                pos_key_layer, dim=-1, index=c2p_pos
            )  # (n_heads, q_len, head_dim, att_span)
            # (bsz, n_heads, q_len, head_dim) * (n_heads, q_len, head_dim, att_span) -> (bsz, n_heads, q_len, att_span)
            c2p_att = torch.matmul(
                query_layer.unsqueeze(3), pos_key_layer
            )  # (bsz, n_heads, q_len, att_span)
    ...
    def forward(
        ...
        attn_outputs = attn_outputs.permute(0, 2, 1, 3).reshape(
            bsz, -1, self.config.dim
        )  # (bsz, seq_len, dim)
