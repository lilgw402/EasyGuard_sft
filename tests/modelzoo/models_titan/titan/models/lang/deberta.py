...
from titan.models.components.bert_components import BertEmbedding, BertEmbeddingPinyin, BertPooler, init_weights, BertPreTrainingHeads
from titan.models.components.transformer_encoder_decoder import BareTransformerEncoderLayer, BareTransformerEncoder, _expand_mask
from titan.models.components.transformer_encoder_decoder import TransformerEncoderLayerOutput, TransformerEncoderOutput
from titan.models.components.disentangled_attention import DisentangledMHA, DAConfig, build_relative_position
...
def deberta_base_6l(pretrained=False, **kwargs):
    ...
    default_config = dict(
        ...
        extra_da_transformer_config=dict(
    ...
    # if pretrained:
    editable_keys = [
        'p_drop_hidden', 'p_drop_attn', 'calc_mlm_accuracy',
        'omit_other_output', 'layernorm_fp16', 'omit_other_attn_output',
        'use_emd', 'dim_shrink']
    for k, v in model_config.items():
        if k == 'extra_da_transformer_config':
            for kk, vv in model_config[k].items():
                if kk in editable_keys:
                    default_config[k][kk] = vv
                else:
                    raise ValueError(f"Cannot edit 'extra_da_transformer_config.{kk}={vv}' with pretrained=True")
        elif k in editable_keys:
            default_config[k] = v
        else:
            raise ValueError(f"Cannot edit '{k}={v}' with pretrained=True")

    model = DebertaModel(**default_config)
...
def deberta_base_12l_erlangshen(pretrained=False, **kwargs):
    ...
    hf_config = DebertaV2Config(**default_config)
    model = DebertaV2ForMaskedLM(hf_config)
...
def deberta_base_6l_pinyin(pretrained=False, **kwargs):
    ...
    default_config = dict(
        vocab_size=145608,
        pinyin_vocab_size=72,
        #pinyin_vocab_size=69,
        # pinyin_vocab_size=32,
        dim=768,
        dim_ff=3072,
        n_segments=2,
        max_len=512,
    ...
    model = DebertaModelPinyin(**default_config)
...
def deberta_base_moe_6l(pretrained=False, **kwargs):
    ...
    default_config = dict(
        ...
        extra_da_transformer_config=dict(
            ...
            use_moe=True,
            use_moe_type='pwff',
            use_moe_transformer_layer='1,3,5',
            moe_k=2,
            moe_experts=32,
            gate_bias=False,
            moe_output_dropout_prob=0.1,
            moe_dropout=False,
            moe_l_aux_factor=0.01,
            moe_dim=768,
            moe_load_balanced=False,
            moe_expert_shape="abc->abd",
            layernorm_fp16=True,
            mha_acts_unite_d01=False,
            omit_other_attn_output=True,
        ),
    ...
    if pretrained:
        editable_keys = [
            'p_drop_hidden', 'p_drop_attn', 'calc_mlm_accuracy',
            'omit_other_output', 'layernorm_fp16', 'omit_other_attn_output',
            'moe_l_aux_factor', 'moe_output_dropout_prob', 'moe_dropout', 'moe_load_balanced']
        for k, v in model_config.items():
            if k == 'extra_da_transformer_config':
                for kk, vv in model_config[k].items():
                    if kk in editable_keys:
