""" Shuffle Transformer
A PyTorch impl of : `"Shuffle Transformer: Rethinking Spatial
Shuffle for Vision Transformer" <https://arxiv.org/abs/2106.03650>`_.

Code from: https://github.com/mulinmeng/Shuffle-Transformer
"""
...
from titan.models.components import get_sinusoid_encoding
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # if self.shuffle:
        #     q, k, v = rearrange(qkv, 'b (qkv h d) (ws1 hh) (ws2 ww)
        #           -> qkv (b hh ww) h (ws1 ws2) d',
        #           h=self.num_heads, qkv=3, ws1=self.ws, ws2=self.ws)
        # else:
        #     q, k, v = rearrange(qkv, 'b (qkv h d) (hh ws1) (ww ws2)
        #           -> qkv (b hh ww) h (ws1 ws2) d',
        #           h=self.num_heads, qkv=3, ws1=self.ws, ws2=self.ws)

        nh, d = self.num_heads, c // self.num_heads
        ws, hh, ww = self.ws, h // self.ws, w // self.ws
        if self.shuffle:
            qkv = qkv.view(b, 3, nh, d, ws, hh, ws, ww)
            qkv = qkv.permute(1, 0, 5, 7, 2, 4, 6, 3).contiguous().view(
                3, (b * hh * ww), nh, (ws * ws), d)
        else:
            qkv = qkv.view(b, 3, nh, d, hh, ws, ww, ws)
            qkv = qkv.permute(1, 0, 4, 6, 2, 5, 7, 3).contiguous().view(
                3, (b * hh * ww), nh, (ws * ws), d)
        ...
        if self.shuffle:
            out = out.view(b, hh, ww, nh, ws, ws, d)
            out = out.permute(0, 3, 6, 4, 1, 5,
                              2).contiguous().view(b, c, h, w)
        else:
            out = out.view(b, hh, ww, nh, ws, ws, d)
            out = out.permute(0, 3, 6, 1, 4, 2,
...
class Block(nn.Module):
    def __init__(
        ...
        self.local = nn.Conv2d(dim, dim, window_size, 1,
                               window_size // 2, groups=dim, bias=qkv_bias)
        ...
        # print("input dim={}, output dim={}, stride={}, expand={},
        #   num_heads={}".format(dim, out_dim, stride, shuffle, num_heads))
...
class StageModule(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert layers % 2 == 0, \
            'Stage layers need to be divisible by 2 for regular' \
            ' and shifted block.'
...
class PatchEmbedding(nn.Module):
    def __init__(self, inter_channel=32, out_channels=48):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(inter_channel, out_channels,
...
class ShuffleTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.out_channels = [dim for dim in dims]
...
@register_model
def shuffletransformer(pretrained=False, **kwargs):
    model_name = 'shuffletransformer'
    pretrained_config, model_config = get_configs(**kwargs)

    model = ShuffleTransformer(**model_config)
    if pretrained:
