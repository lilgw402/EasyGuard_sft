...
__all__ = ['convnext_tiny', 'convnext_small',
...
class ConvNeXt(nn.Module):
    ...
    Args:
        ...
        layer_scale_init_value (float): Init value for Layer Scale.
            Default: 1e-6.
        head_init_scale (float): Init scaling value for classifier
            weights and biases. Default: 1.
        global_pool (str): pooling type from the last conv feature map,
            default is 'avg'. If don't need global pooling,
            just set this to None or ''.
...
def convnext_tiny(pretrained=False, **kwargs):
    ...
    model = ConvNeXt(depths=[3, 3, 9, 3], dims=[
                    96, 192, 384, 768], name=model_name, **model_config)
    if pretrained:
...
def convnext_base(pretrained=False, **kwargs):
    ...
    model = ConvNeXt(depths=[3, 3, 27, 3], dims=[
                    128, 256, 512, 1024], name=model_name, **model_config)
    if pretrained:
        weight_path = download_weights(
            model_name,
            **pretrained_config)
...
def convnext_large(pretrained=False, **kwargs):
    ...
    model = ConvNeXt(depths=[3, 3, 27, 3], dims=[
                    192, 384, 768, 1536], name=model_name, **model_config)
    if pretrained:
        weight_path = download_weights(
            model_name,
            **pretrained_config)
...
def convnext_xlarge(pretrained=False, **kwargs):
    model_name = 'convext_xlarge'
    pretrained_config, model_config = get_configs(**kwargs)

    model = ConvNeXt(depths=[3, 3, 27, 3], dims=[
                    256, 512, 1024, 2048], name=model_name,
                    **model_config)
    if pretrained:
        weight_path = download_weights(
            model_name,
