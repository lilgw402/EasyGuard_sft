...
Code from: https://github.com/SwinTransformer/Swin-Transformer-Object-Detection
"""
...
class WindowAttention(nn.Module):
    r""" Window based multi-head self attention (W-MSA) module with
     relative position bias. It supports both shifted and non-shifted window.
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        # nW*B, window_size*window_size, C
        attn_windows, attn_map = self.attn(x_windows, mask=attn_mask)
    ...
    def forward(self, x, H, W):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                x, attn_map = checkpoint.checkpoint(blk, x, attn_mask)
            else:
                x, attn_map = blk(x, attn_mask)

        outs = []
        if self.downsample is not None:
            x_down = self.downsample(x, H, W)
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
            outs.extend([x, H, W, x_down, Wh, Ww])
        else:
            outs.extend([x, H, W, x, H, W])
...
class SwinTransformer(nn.Module):
    r"""
    A PyTorch impl of : `"Swin Transformer: Hierarchical Vision
    Transformer using Shifted Windows" <https://arxiv.org/pdf/2103.14030>`_.

    Args:
        pretrain_img_size (int | tuple(int)): Input image size for training
            the pretrained model, used in absolute postion embedding.
            Default 224.
        ...
        use_checkpoint (bool): Whether to use checkpointing to save memory.
            Default: False
        global_pool (str): pooling type from the last conv feature map,
            default is 'avg'. If don't need global pooling, just set this to
            None or ''.
    ...
    def __init__(
        ...
        '''if i_layer < self.num_layers - 1:
            self.out_channels.append(int(embed_dim * 2 ** i_layer) * 2)
        else:
            self.out_channels.append(int(embed_dim * 2 ** i_layer))'''
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x_out, H, W, x, Wh, Ww, attn_map = layer(x, Wh, Ww)
            if i in self.out_indices:
                norm_layer = getattr(self, f'norm{i}')
                x_out = norm_layer(x_out)

                out = x_out.view(-1, H, W, self.out_channels[i]).permute(
                            0, 3, 1, 2).contiguous()
                outs[i] = out
...
@register_model
def swin_tiny_det(pretrained=False, **kwargs):
    model_name = 'swin_tiny_det'
...
@register_model
def swin_small_det(pretrained=False, **kwargs):
    model_name = 'swin_small_det'
    pretrained_config, model_config = get_configs(**kwargs)
...
@register_model
def swin_base_det(pretrained=False, **kwargs):
    model_name = 'swin_base_det'
    pretrained_config, model_config = get_configs(**kwargs)
...
@register_model
def swin_large_det(pretrained=False, **kwargs):
    model_name = 'swin_large_det'
    pretrained_config, model_config = get_configs(**kwargs)
