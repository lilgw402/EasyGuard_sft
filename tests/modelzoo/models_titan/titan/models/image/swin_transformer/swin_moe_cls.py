...
__all__ = ['swin_base_moe', 'swin_large_moe']
...
class MoEMlp(nn.Module):
    def __init__(self, in_features, hidden_features, num_local_experts, top_value, capacity_factor=1.25,
                 ...
                 mlp_fc2_bias=True, split_ffn=False):
        super().__init__()
        try:
            import janus.layer
            import logging
            logging.getLogger("janus").setLevel(logging.ERROR)
        except ImportError:
            msg = "Janus has not been installed. To use Swin-MoE, please install Janus from scm: aml/mlsys/janus_dev"
            msg += ' You can also install by pip3 install -U byted-janus -i "https://bytedpypi.byted.org/simple" for example.'
        ...
        self.split_ffn = split_ffn
        self._dropout = nn.Dropout(p=moe_drop)
        if cosine_router:
            raise RuntimeError("Not supported cosine top moe")
        if self.split_ffn:
            expert1 = torch.nn.Linear(in_features, hidden_features)
            expert2 = torch.nn.Linear(hidden_features, in_features, bias=self.mlp_fc2_bias)
            self._moe_layer1 = janus.layer.MoE(
                hidden_size=in_features,
                expert=expert1,
                num_experts=num_local_experts,  # 不改这里的名字了，实际上传的是全局的num_experts
                k=top_value,
                capacity_factor=capacity_factor,
                noisy_gate_policy='Jitter')
            self._moe_layer2 = janus.layer.MoE(
            ...
            expert = torch.nn.Sequential(
                ...
                nn.GELU(),
                self._dropout,
                nn.Linear(hidden_features, in_features, bias=self.mlp_fc2_bias),
                nn.GELU(),
                self._dropout
                )
            self._moe_layer = janus.layer.MoE(
                hidden_size=in_features,
                expert=expert,
                num_experts=num_local_experts,  # 不改这里的名字了，实际上传的是全局的num_experts
                k=top_value,
    ...
    def forward(self, x):
        if self.split_ffn:
            x, l_aux1, _ = self._moe_layer1(x)
            x = self._dropout(F.gelu(x))
            x, l_aux2, _ = self._moe_layer2(x)
            x = self._dropout(F.gelu(x))
            return x, l_aux1 + l_aux2
        else:
            x, l_aux, _ = self._moe_layer(x)
            return x, l_aux
        ...
        def init_weights(module):
            for k, v in module.named_parameters():
                if 'experts' not in k:
                    continue
                if 'weight' in k:
                    trunc_normal_(v, std=self.init_std)
                elif 'bias' in k:
                    nn.init.constant_(v, 0)
        if self.split_ffn:
            init_weights(self._moe_layer1.moe)
            init_weights(self._moe_layer2.moe)
        else:
            init_weights(self._moe_layer.moe)
...
class SwinTransformerBlock(nn.Module):
    ...
    Args:
        ...
        split_moe_ffn: split moe ffn into two linear layers
    """
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 cosine_router_dim=256, cosine_router_init_t=0.5, moe_drop=0.0, split_moe_ffn=False):
        super().__init__()
        self.dim = dim
...
class SwinTransformerMoE(nn.Module):
    ...
    Args:
        ...
        mlp_fc2_bias (bool): Whether to add bias in fc2 of Mlp. Default: False
        init_std: Initialization std. Default: 0.02
        use_checkpoint (bool): Whether to use checkpointing to save memory. Default: False
        ...
        disable_gate_loss (bool): disable gate loss during forward.
            This argument has same effect as forward(self, x, gate_loss=False) but can be configured during init.
    """
    __model_type__ = 'vit'
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 mlp_fc2_bias=False, init_std=0.02, use_checkpoint=False, pretrained_window_sizes=[0, 0, 0, 0],
                 ...
                 features_only=False, name=None, global_pool='avg', use_attn_map=False, disable_gate_loss=False,
                 **kwargs):
        super().__init__()
        self._disable_gate_loss = disable_gate_loss
        ...
        self.num_local_experts = num_local_experts
        self.global_experts = num_local_experts
        ...
        if self.out_indices is None:
            self.out_indices = [i for i in range(self.num_layers)]
    ...
    def forward_features(self, x, gate_loss=True):
        if self._disable_gate_loss:
            gate_loss = False
        outs = OrderedDict()
        ...
        for i, layer in enumerate(self.layers):
            x, cur_l_aux, attn_map = layer(x)  # B L C
            l_aux = cur_l_aux + l_aux
            if self.features_only and i in self.out_indices:
                B, L, C = x.shape
                S = int(sqrt(L))
                assert S ** 2 == L, "feature map is not square."
                outs[i] = x.transpose(1, 2).reshape(B, C, S, S)

        if self.features_only:
            if gate_loss:
                return outs, l_aux, attn_map
            else:
    ...
    def forward(self, x, gate_loss=True):
        if self._disable_gate_loss:
            gate_loss = False
        out = self.forward_features(x, gate_loss=True)

        if self.features_only:
            x, l_aux, attn_map = out
            l_aux *= self.aux_loss_weight

            if self.use_attn_map and self.training:
                return list(x.values()), l_aux, attn_map if gate_loss else list(x.values()), attn_map
            else:
                return list(x.values()), l_aux if gate_loss else list(x.values())
        else:
            x = self.head(out[0])
            l_aux = out[1]
            ret = [x]
            if gate_loss:
                ret.append(l_aux * self.aux_loss_weight)
            if self.use_attn_map and self.training:
                attn_map = out[2]
                ret.append(attn_map)
            if len(ret) == 1:
...
@register_model
def swin_base_moe(pretrained=False, **kwargs):
    model_name = 'swin_base_moe'
    pretrained_config, model_config = get_configs(**kwargs)

    img_size = model_config.pop('img_size', 224)
    moe_blocks = model_config.pop('moe_blocks', [[-1], [-1], [1, 3, 5, 7, 9, 11, 13, 15, 17], [1]])
    pretrained_window_sizes = model_config.pop('pretrained_window_sizes', [0, 0, 0, 0])
    window_size = model_config.pop('window_size', img_size // 16)
    num_experts = model_config.pop('num_experts', 16)
    topk = model_config.pop('top_value', 1)
...
@register_model
def swin_large_moe(pretrained=False, **kwargs):
    model_name = 'swin_large_moe'
    pretrained_config, model_config = get_configs(**kwargs)
    ...
    pretrained_window_sizes = model_config.pop('pretrained_window_sizes', [0, 0, 0, 0])
    window_size = model_config.pop('window_size', img_size // 16)
    num_experts = model_config.pop('num_experts', 4)
    topk = model_config.pop('top_value', 1)
