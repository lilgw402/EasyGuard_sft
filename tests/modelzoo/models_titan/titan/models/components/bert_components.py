...
from titan.models.components.activation import Activation
from titan.models.components.layernorm import LayerNorm, LayerNormTypes
...
class PinyinEmbedding(nn.Module):
    def __init__(self, embedding_size: int, pinyin_out_dim: int, pinyin_vocab_size: int):
        ...
        self.conv1 = nn.Conv1d(in_channels=embedding_size, out_channels=self.pinyin_out_dim, kernel_size=1,
                              stride=1, padding=0)
    ...
    def forward(self, pinyin_ids):
        ...
        view_pinyin_embed = pinyin_embed1.view(bs, sentence_length, self.pinyin_out_dim) # [bs,sentence_length,pinyin_out_dim]
...
class PinyinEmbeddingV2(nn.Module):
    def __init__(self, embedding_size: int, pinyin_out_dim: int, pinyin_vocab_size: int):
        ...
        self.conv1 = nn.Conv1d(in_channels=embedding_size, out_channels=self.pinyin_out_dim, kernel_size=1,
                              stride=1, padding=0)
        self.conv2 = nn.Conv1d(in_channels=embedding_size, out_channels=self.pinyin_out_dim, kernel_size=2,
                              stride=1, padding=0)
    ...
    def forward(self, pinyin_ids):
        ...
        view_pinyin_embed = pinyin_embed.view(bs, sentence_length, self.pinyin_out_dim*2) # [bs,sentence_length,pinyin_out_dim*2]
...
class BertEmbeddingPinyin(torch.nn.Module):
    ...
    def __init__(
        ...
        # pinyin Embeddings
        self.pinyin_embeddings = PinyinEmbeddingV2(embedding_size=pinyin_embedding_dim,
                                                 pinyin_out_dim=dim,
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, mask=None, embed_only: bool = False, pinyin_ids=None):
        if isinstance(input_ids, dict):
            token_type_ids = input_ids.get('segments')
            position_ids = input_ids.get('positions')
            inputs_embeds = input_ids.get('token_embeddings')
