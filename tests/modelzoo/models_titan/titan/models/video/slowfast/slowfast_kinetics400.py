...
__all__ = [
    'slowfast_4x16_resnet50',
    'slowfast_8x8_resnet50',
    'slowfast_4x16_resnet101',
    'slowfast_8x8_resnet101',
]
    ...
    def __init__(self,
        ...
        self.lateral_res2 = nn.Sequential(
            nn.Conv3d(in_channels=self.width_per_group * 4 // self.beta_inv,
                      out_channels=self.width_per_group *
                      4 // self.beta_inv * self.fusion_conv_channel_ratio,
                      kernel_size=(self.fusion_kernel_size, 1, 1),
                      stride=(self.alpha, 1, 1),
                      padding=(self.fusion_kernel_size // 2, 0, 0),
                      bias=False),
        ...
        self.lateral_res3 = nn.Sequential(
            nn.Conv3d(in_channels=self.width_per_group * 8 // self.beta_inv,
                      out_channels=self.width_per_group *
                      8 // self.beta_inv * self.fusion_conv_channel_ratio,
                      kernel_size=(self.fusion_kernel_size, 1, 1),
                      stride=(self.alpha, 1, 1),
                      padding=(self.fusion_kernel_size // 2, 0, 0),
                      bias=False),
        ...
        self.lateral_res4 = nn.Sequential(
            nn.Conv3d(in_channels=self.width_per_group * 16 // self.beta_inv,
                      out_channels=self.width_per_group *
                      16 // self.beta_inv * self.fusion_conv_channel_ratio,
                      kernel_size=(self.fusion_kernel_size, 1, 1),
                      stride=(self.alpha, 1, 1),
                      padding=(self.fusion_kernel_size // 2, 0, 0),
                      bias=False),
        ...
        self.feat_dim = self.width_per_group * \
            32 // self.beta_inv + self.width_per_group * 32
...
@register_model
def slowfast_4x16_resnet50(pretrained=False, **kwargs):
    global_pool = kwargs.pop('global_pool', 'avg')
    assert global_pool == 'avg', \
        'SlowFast model must have global_pool type:avg.'
    model_name = 'slowfast_4x16_resnet50'
    pretrained_config, model_config = get_configs(**kwargs)
    if model_config['features_only']:
        raise ValueError(
            "slowfast models does not support features_only.")
    model_config.pop('features_only')
    model = SlowFast(layers=[3, 4, 6, 3],
                     num_segment=1,
                     num_crop=1,
                     alpha=8,
                     beta_inv=8,
...
@register_model
def slowfast_8x8_resnet50(pretrained=False, **kwargs):
    global_pool = kwargs.get('global_pool', 'avg')
    assert global_pool == 'avg', \
        'SlowFast model must have global_pool type:avg.'
    model_name = 'slowfast_8x8_resnet50'
    pretrained_config, model_config = get_configs(**kwargs)
    if model_config['features_only']:
        raise ValueError(
            "slowfast models does not support features_only.")
...
@register_model
def slowfast_4x16_resnet101(pretrained=False, **kwargs):
    global_pool = kwargs.get('global_pool', 'avg')
    assert global_pool == 'avg', \
        'SlowFast model must have global_pool type:avg.'
    model_name = 'slowfast_4x16_resnet101'
    _, model_config = get_configs(**kwargs)
    if model_config['features_only']:
        raise ValueError(
            "slowfast models does not support features_only.")
    model_config.pop('features_only')
    model = SlowFast(layers=[3, 4, 23, 3],
                     num_segment=1,
                     num_crop=1,
                     alpha=8,
                     beta_inv=8,
...
@register_model
def slowfast_8x8_resnet101(pretrained=False, **kwargs):
    global_pool = kwargs.get('global_pool', 'avg')
    assert global_pool == 'avg', \
        'SlowFast model must have global_pool type:avg.'
    model_name = 'slowfast_8x8_resnet101'
    pretrained_config, model_config = get_configs(**kwargs)
    if model_config['features_only']:
        raise ValueError(
            "slowfast models does not support features_only.")
