...
Copyright 2018-2019 Open-MMLab, Apache License, Version 2.0
"""
...
__all__ = ['video_swin_t', 'video_swin_s', 'video_swin_b', 'video_swin_l']
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        attn_windows, attn_map = self.attn(
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.use_checkpoint:
            x, attn_map = checkpoint.checkpoint(self.forward_part1, x, mask_matrix)
        else:
            x, attn_map = self.forward_part1(x, mask_matrix)
...
class SwinTransformer3D(nn.Module):
    ...
    def inflate_weights(self, pretrained_path):
        ...
        with open(pretrained_path, 'rb') as f:
            ...
            elif checkpoint.get('model', None):
...
def update_user_cfg(cfg, **kwargs):
    supported_keys = [
        'patch_size',
        'in_chans',
        'embed_dim',
...
@register_model
def video_swin_t(pretrained=False, pretrained2d=False, **kwargs):
    model_name = 'video_swin_t'
    cfg = _default_cfg[model_name]
    cfg['name'] = model_name
    update_user_cfg(cfg, **kwargs)
    model = SwinTransformer3D(**cfg)
    if pretrained:
        pretrained_config, _ = get_configs(**kwargs)
        weight_path = download_weights(
            model_name,
            **pretrained_config)
        if pretrained2d:
            model.inflate_weights(weight_path)
        else:
...
@register_model
def video_swin_s(pretrained=False, pretrained2d=False, **kwargs):
    model_name = 'video_swin_s'
    cfg = _default_cfg[model_name]
    cfg['name'] = model_name
    update_user_cfg(cfg, **kwargs)
    model = SwinTransformer3D(**cfg)
...
@register_model
def video_swin_b(pretrained=False, pretrained2d=False, **kwargs):
    model_name = 'video_swin_b'
    cfg = _default_cfg[model_name]
    cfg['name'] = model_name
    update_user_cfg(cfg, **kwargs)
    model = SwinTransformer3D(**cfg)
...
@register_model
def video_swin_l(pretrained=False, pretrained2d=False, **kwargs):
    model_name = 'video_swin_l'
    cfg = _default_cfg[model_name]
    cfg['name'] = model_name
    update_user_cfg(cfg, **kwargs)
    model = SwinTransformer3D(**cfg)
