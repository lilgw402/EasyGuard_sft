...
def pad_framewise_output(framewise_output, frames_num):
    ...
    """
    pad = framewise_output[:, -
                           1:, :].repeat(1, frames_num -
...
def count_flops(model, audio_length):
    ...
    def foo(net):
        ...
        if not childrens:
            ...
            elif isinstance(net, nn.BatchNorm2d) or \
                    isinstance(net, nn.BatchNorm1d):
                net.register_forward_hook(bn_hook)
            elif isinstance(net, nn.ReLU):
                net.register_forward_hook(relu_hook)
            elif isinstance(net, nn.AvgPool2d) or \
                    isinstance(net, nn.MaxPool2d):
                net.register_forward_hook(pooling2d_hook)
            elif isinstance(net, nn.AvgPool1d) or \
                    isinstance(net, nn.MaxPool1d):
    ...
    total_flops = sum(list_conv2d) + sum(list_conv1d) + sum(list_linear) + \
        sum(list_bn) + sum(list_relu) + \
        sum(list_pooling2d) + sum(list_pooling1d)
