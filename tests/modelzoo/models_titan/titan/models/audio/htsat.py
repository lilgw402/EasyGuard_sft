...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, 
                patch_size=4, 
                patch_stride=(4,4), 
                in_chans=1, 
                num_classes=50,
        ...
        self.enable_tscam = enable_tscam
        ...
        self.spectrogram_extractor = Spectrogram(n_fft=spectro_window_size, hop_length=hop_size, 
            win_length=spectro_window_size, window=window, center=center, pad_mode=pad_mode, 
            freeze_parameters=True)
        # Logmel feature extractor
        self.logmel_extractor = LogmelFilterBank(sr=sample_rate, n_fft=spectro_window_size, 
            n_mels=self.mel_bins, fmin=fmin, fmax=fmax, ref=ref, amin=amin, top_db=top_db, 
            freeze_parameters=True)
        # Spec augmenter
...
@register_model
def hts_at_model(pretrained=False, **kwargs):
    tos_helper = kwargs.pop('tos_helper', None)
    pretrained_version = kwargs.pop('pretrained_version', None)
    pretrained_uri = kwargs.pop('pretrained_uri', None)
    model_name = 'hts_at_model'
    model = HTSAT_Swin_Transformer(**kwargs)
    ...
    if pretrained:
        ...
        sed_wrapper = SEDWrapper(sed_model = model)
