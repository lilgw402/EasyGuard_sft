...
try:
    ...
    print('%s did not found torch_io module, please check import path'% e)
    torch_io_load = torch.load
...
def resnet18(pretrained=False, pretrained_model_path=None, num_classes=None, expose_stages=None, dilations=None, **kwargs):
    ...
    if pretrained:
        if pretrained_model_path is not None:
            state_dict = torch_io_load(pretrained_model_path, map_location=lambda storage, loc: storage)
            print('load resnet backbone from %s' % pretrained_model_path)
            model.load_pretrained_state_dict(state_dict)
        else:
            print('no pretrained model, training resnet FROM SCRATCH')
            # state_dict = model_zoo.load_url(model_urls['resnet18'])
            # print('download and load resnet backbone from %s' % pretrained_model_path)
    return model
