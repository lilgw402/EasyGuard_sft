...
from fex.utils.torch_io import load as torch_load
from fex import _logger as logger
from fex.nn.backbone.dino import DropPath, trunc_normal_
...
class SwinTransformer3D(nn.Module):
    ...
    def inflate_weights(self):
        ...
        """
        state_dict = torch_load(self.pretrained, map_location='cpu')
    ...
    def forward(self, x, return_dict=False):
        ...
        avg = self.avgpool(x).squeeze(-1).squeeze(-1).squeeze(-1)
        avg = self.head(avg)

        if return_dict:
            return {'feature_map': x, 'pooled_out': avg}
        else:
            return avg
...
def swin_transformer_3d_B244():
    """
    """

    return SwinTransformer3D(patch_size=(2, 4, 4),
                             embed_dim=128,
                             depths=[2, 2, 18, 2],
                             num_heads=[4, 8, 16, 32]
                             )
