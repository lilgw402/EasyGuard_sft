...
# from https://code.byted.org/zengyan.yanne/X-VLM/blob/dev_swin_base_lr_github_more/models/clip_vit.py
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        outputs, attn_weights = self.encoder(
        ...
        if return_dict:
            return {'feature_map': outputs, 'pooled_out': outputs[:, 0, :], 'attn_weights': attn_weights}
        else:
            return outputs
...
def clip_vit_b16(image_size, **kwargs):
    model = CLIPVisionTransformer(image_size,
                                  patch_size=16,
                                  hidden_size=768,
