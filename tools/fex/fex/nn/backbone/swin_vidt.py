...
def masked_sin_pos_encoding(x, mask, num_pos_feats, temperature=10000, scale=2 * math.pi):
    ...
    dim_t = temperature ** (2 * (dim_t // 2) / num_pos_feats)  # TODO: __floordiv__
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, mask=None, *args, **kwargs):
        ...
        if mask is None:
            mask = torch.zeros([B, ori_H, ori_W], device=x.device)
        ...
        avg = torch.mean(patch_outs[-1], dim=1)

        return {'feature_map': patch_outs[-1],
                'encoded_layers': patch_outs,
                'pooled_out': avg,
                'det_tgt': det_tgt,
                'det_pos': det_pos
                }
...
def swin_base_win7_vidt(det_token_num=300, reduced_dim=256, cross_indices=[3], **kwargs):
    ...
    model.finetune_det(method='vidt',
