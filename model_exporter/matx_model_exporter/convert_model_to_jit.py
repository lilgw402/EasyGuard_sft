...
def trace_model():
    ...
    model = DebertaClassifier(config=trace_configs.model_name_or_path, num_labels=trace_configs.num_classes)
    ...
    traced_model = torch.jit.trace(model, (input_ids.to(device),  segment_ids.to(device), attention_mask.to(device)))
...
if __name__ == "__main__":
    ...
    jit_model = torch.jit.load(os.path.join(trace_configs.output_path, 'model.jit'), map_location=device)
